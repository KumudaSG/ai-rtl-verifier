...
TOOL_CONFIGS = {
    ...
    "P6": {"testbench_file": "tb_p6.sv"},
    "P7": {"testbench_file": "tb_p7.sv"},
    "P8": {"testbench_file": "tb_p8.sv"},
    "P9": {"testbench_file": "tb_p9.sv"},
    "P10": {"testbench_file": "tb_p10.sv"}
}
...
def write_solution_files(problem_id: str, generated_solution: GeneratedSolution) -> List[str]:
    ...
    if generated_solution.memory_file is not None:
        memory_filename = f"memory_{problem_tag}.sv"
        write_file(memory_filename, generated_solution.memory_file.content)
        written_files.append(memory_filename)

    if generated_solution.pointer_file is not None:
        pointer_filename = f"pointer_{problem_tag}.sv"
        write_file(pointer_filename, generated_solution.pointer_file.content)
        written_files.append(pointer_filename)

    if generated_solution.selector_file is not None:
        selector_filename = f"selector_{problem_tag}.sv"
        write_file(selector_filename, generated_solution.selector_file.content)
        written_files.append(selector_filename)

    if generated_solution.decoder_file is not None:
        decoder_filename = f"decoder_{problem_tag}.sv"
        write_file(decoder_filename, generated_solution.decoder_file.content)
        written_files.append(decoder_filename)

    if generated_solution.accumulator_file is not None:
        accumulator_filename = f"accumulator_{problem_tag}.sv"
        write_file(accumulator_filename, generated_solution.accumulator_file.content)
        written_files.append(accumulator_filename)
